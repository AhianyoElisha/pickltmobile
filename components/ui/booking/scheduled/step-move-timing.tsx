...
function Checkbox({ checked }: { checked: boolean }) {
  return (
    <View style={[cb.box, checked ? cb.boxChecked : cb.boxUnchecked]}>
      {checked && (
...
const cr = StyleSheet.create({
  ...
  label: { fontFamily: FontFamily.regular, fontSize: 16, lineHeight: 25.6, color: Colors.textPrimary, flex: 1 },
});
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.fieldGroup}>
        <Text style={s.fieldLabel}>Move date</Text>
        <View style={s.dateDisplay}>
          <Text style={s.dateText}>{formattedDate || 'No date selected'}</Text>
        </View>
      </View>
      ...
      <View style={s.optionalSection}>
        <Text style={s.optionalTitle}>Optional Preferences</Text>
        <Text style={s.optionalSubtitle}>Select any additional timing preferences.</Text>
...
const s = StyleSheet.create({
  ...
  fieldGroup: { gap: 8 },
  fieldLabel: {
    fontFamily: FontFamily.medium, fontSize: 16, lineHeight: 22.4, color: Colors.textPrimary,
  },
  dateDisplay: {
    backgroundColor: '#F0F0F0', borderRadius: 12, paddingHorizontal: 16, paddingVertical: 15,
  },
  dateText: {
    fontFamily: FontFamily.regular, fontSize: 16, lineHeight: 22.4, color: Colors.textSecondary,
  },

  optionalSection: { gap: 4 },
  optionalTitle: {
    fontFamily: FontFamily.semibold, fontSize: 16, lineHeight: 22.4, color: Colors.textPrimary,
  },
  optionalSubtitle: {
    fontFamily: FontFamily.regular, fontSize: 14, lineHeight: 21.7, color: Colors.textSecondary, marginBottom: 8,
  },
  checkboxList: { gap: 12 },
});
